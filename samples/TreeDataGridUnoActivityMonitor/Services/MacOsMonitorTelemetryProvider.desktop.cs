...
using static TreeDataGridUnoActivityMonitor.Services.MonitorFormatters;
...
internal sealed class MacOsMonitorTelemetryProvider : IMonitorTelemetryProvider
{
    ...
    private readonly double _nanosecondsPerTick = ReadNanosecondsPerTick();
    ...
    private MonitorSnapshot CaptureSnapshot(CancellationToken cancellationToken)
    {
        ...
        var elapsedSeconds = Math.Max((now - (_previousTimestamp ?? now.AddSeconds(-1))).TotalSeconds, 0.001);
    ...
    private CpuSummary SampleCpuLoad()
    {
        ...
        var result = host_processor_info(host, ProcessorCpuLoadInfo, out _, out var processorInfoPtr, out var processorInfoCount);
        ...
        {
            _ = vm_deallocate(mach_task_self(), processorInfoPtr, (nuint)(processorInfoCount * sizeof(int)));
        }
    }
    ...
    private MemorySummary SampleMemory()
    {
        ...
        var host = mach_host_self();
        var result = host_statistics64(host, HostVmInfo64, ref vmStats, ref count);
        _ = mach_port_deallocate(mach_task_self(), host);
        if (result != 0)
    ...
    private VolumeSummary SampleVolumes()
    {
        ...
        try
        {
            ...
            for (var i = 0; i < Math.Min(actualCount, count); ++i)
            {
                var stat = Marshal.PtrToStructure<StatFs>(buffer + (i * entrySize));
                // APFS volumes can share storage: report only the root volume.
                if (stat.MountOnName != "/") continue;
                var bytesTotal = stat.Blocks * stat.BlockSize;
                var bytesFree = stat.BlockAvailable * stat.BlockSize;
                return new VolumeSummary(bytesTotal, bytesTotal > bytesFree ? bytesTotal - bytesFree : 0);
            }
    ...
    private List<NetworkMonitorRow> SampleInterfaces(double elapsedSeconds)
    {
        ...
        try
        {
            for (var current = ifap; current != IntPtr.Zero; current = Marshal.PtrToStructure<IfAddrs>(current).Next)
            {
                ...
                var rxPerSecond = previous is null ? 0 : CounterDelta(received, previous.InputBytes) / elapsedSeconds;
                var txPerSecond = previous is null ? 0 : CounterDelta(sent, previous.OutputBytes) / elapsedSeconds;
        ...
        foreach (var stale in _previousNetworkCounters.Keys.Where(x => !seen.Contains(x)).ToArray())
            _previousNetworkCounters.Remove(stale);
        return rows;
    }
    ...
    private ProcessSnapshot SampleProcesses(double elapsedSeconds, CancellationToken cancellationToken)
    {
        ...
        // Allow for process starts between sizing and filling the buffer.
        var capacity = checked(processes + 128);
        var buffer = Marshal.AllocHGlobal(checked(capacity * sizeof(int)));
        var activeKeys = new HashSet<int>();

        try
        {
            var returned = proc_listallpids(buffer, capacity * sizeof(int));
            if (returned <= 0)
                return new ProcessSnapshot([], [], [], []);

            var count = Math.Min(returned, capacity);
            ...
            foreach (var pid in pids)
            {
                ...
                if (proc_pidinfo(pid, ProcPidTaskAllInfo, 0, ref info, Marshal.SizeOf<ProcTaskAllInfo>()) != Marshal.SizeOf<ProcTaskAllInfo>())
                    continue;
                ...
                if (previous?.StartTime != usage.ProcessStartAbsoluteTime) previous = null;

                var totalCpuTime = usage.UserTime + usage.SystemTime;
                var cpuDelta = previous is null ? 0 : CounterDelta(totalCpuTime, previous.CpuTime);
                var cpuPercent = cpuDelta * _nanosecondsPerTick / (elapsedSeconds * 10_000_000d);
                var userDelta = previous is null ? 0 : CounterDelta(usage.UserTime, previous.UserTime);
                var userPercent = userDelta * _nanosecondsPerTick / (elapsedSeconds * 10_000_000d);

                var idleWakeups = previous is null ? 0 : CounterDelta(usage.PackageIdleWakeups, previous.PackageIdleWakeups) / elapsedSeconds;
                var interruptWakeups = previous is null ? 0 : CounterDelta(usage.InterruptWakeups, previous.InterruptWakeups) / elapsedSeconds;
                var readPerSecond = previous is null ? 0 : CounterDelta(usage.DiskReadBytes, previous.ReadBytes) / elapsedSeconds;
                var writePerSecond = previous is null ? 0 : CounterDelta(usage.DiskWriteBytes, previous.WriteBytes) / elapsedSeconds;
                var energyPerSecond = previous is null ? 0 : CounterDelta(usage.EnergyNanoJoules, previous.EnergyNanoJoules) / elapsedSeconds;
                ...
                _previousProcessCounters[pid] = new ProcessCounters(
                    usage.ProcessStartAbsoluteTime,
                    totalCpuTime,
                    usage.UserTime,
                    usage.DiskReadBytes,
                ...
                memoryRows.Add(new MemoryMonitorRow(
                    ...
                    usage.ResidentSize,
                    null, // proc_pid_rusage does not expose per-process compression.
                    usage.PageIns));
    ...
    {
        var registered = ReadNullTerminated(name);
        if (!string.IsNullOrWhiteSpace(registered))
            return registered;

        return ReadNullTerminated(command);
    }
    ...
    private static ulong ReadUnsignedLong(string name)
    {
        ...
        try
        {
            ...
            return length == sizeof(uint) ? (uint)Marshal.ReadInt32(ptr) :
                length == sizeof(ulong) ? (ulong)Marshal.ReadInt64(ptr) : 0;
        }
        finally
        {
    ...
    private static double ReadNanosecondsPerTick()
    {
        if (mach_timebase_info(out var timebase) != 0 || timebase.Denominator == 0)
            throw new InvalidOperationException("Unable to read the Mach CPU timebase.");
        return (double)timebase.Numerator / timebase.Denominator;
    }
