...
namespace Avalonia.Controls.TreeDataGridTests
{
    public class CoreCellSelectionTests
    {
        ...
        [AvaloniaFact]
        public void V12_selection_mode_creates_native_cells_and_reports_cell_deltas()
        {
            using var source = Flat();
            var grid = new TreeDataGrid
            {
                SelectionMode = TreeDataGridSelectionMode.Cell | TreeDataGridSelectionMode.Multiple,
                Model = source
            };
            var selection = Assert.IsType<Core.Selection.TreeDataGridCellSelectionModel<Node>>(source.Selection);
            Assert.False(selection.SingleSelect);
            var window = new TestWindow(grid);
            try
            {
                TreeDataGridSelectionChangedEventArgs? changed = null;
                grid.SelectionChanged += (_, e) => changed = e;
                selection.SelectedIndex = new(1, 0);
                Assert.Equal(new CellIndex(1, 0), Assert.Single(changed!.SelectedCellIndexes));
                selection.SelectedIndex = new(0, 1);
                Assert.Equal(new CellIndex(1, 0), Assert.Single(changed!.DeselectedCellIndexes));
                Assert.Equal(new CellIndex(0, 1), Assert.Single(changed.SelectedCellIndexes));
                grid.SelectionMode = TreeDataGridSelectionMode.Row;
                Assert.IsType<Core.Selection.TreeDataGridRowSelectionModel<Node>>(source.Selection);
                changed = null;
                selection.Clear();
                Assert.Null(changed);
                grid.SelectionMode = TreeDataGridSelectionMode.Cell;
                Assert.True(Assert.IsType<Core.Selection.TreeDataGridCellSelectionModel<Node>>(source.Selection).SingleSelect);
            }
            finally { window.Close(); selection.Dispose(); }
        }
