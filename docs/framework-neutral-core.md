...
On Avalonia 12, an explicitly set `SelectionMode` creates/configures Core row selection directly, and `TreeDataGrid.SelectionChanged` forwards Core selection changes. Declarative `ItemsSource`/`ColumnDefinitions` and legacy cell selection remain supported through `Source`. Core currently supports row selection; requesting cell selection with `Model` fails explicitly.
