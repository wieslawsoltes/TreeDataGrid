#!/usr/bin/env python3
"""Measure the same public accessor harness against exact revisions in ABBA order.
...
WORKLOADS = {
    "owner-link-chain": (32768, 3),
    "custom-column-select": (65536, 17),
    "custom-column-compare": (32768, -1),
    "typed-binding-notification": (4096, 17.5),
    "column-construction": (16, 17),
}
HARNESS = "benchmarks/TreeDataGrid.AccessorExecution"
FILES = ("TreeDataGrid.AccessorExecution.csproj", "Program.cs")
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("artifacts/accessor-comparison"))
    ...
    provenance = {
        ...
        "method": "Two exact unmodified source revisions, identical harness outside each worktree, ABBA process order.",
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="tdg-accessor-") as scratch_name:
            ...
            for ordinal, label in enumerate(order):
                ...
                with (output / f"{ordinal:02d}-{label}.log").open("w") as log:
                    subprocess.run([
                        "dotnet", str(hosts[label] / "bin/Release/net10.0/TreeDataGrid.AccessorExecution.dll"),
                        revisions[label], str(path),
                    ], cwd=hosts[label], stdout=log, stderr=subprocess.STDOUT, check=True, timeout=120)
                report = json.loads(path.read_text())
                if report["revision"] != revisions[label] or len(report["samples"]) != 75:
                ...
                environment = {key: report[key] for key in (
                    ...
                    "samplesPerWorkload", "warmupBatches", "scope")}
                ...
                print("UNO_ACCESSOR_PASS=" + json.dumps({"ordinal": ordinal, "label": label, "revision": revisions[label]}), flush=True)
            ...
            print("UNO_ACCESSOR_COMPARISON=" + json.dumps(result), flush=True)
            return 0
