...
An `IndexPath` is essentially an array of indexes, each element of which specifies the index at a successively deeper level in the hierarchy of the data.
...
By default, selection is single-select. To enable multiple selection set the `SingleSelect` property to `false`, e.g.:
...
The `CellIndex` struct identifies an individual cell by combining an integer column index and an `IndexPath` row index.
