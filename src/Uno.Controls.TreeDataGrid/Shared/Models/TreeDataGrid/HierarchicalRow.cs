...
namespace Avalonia.Controls.Models.TreeDataGrid
{
        ...
        IDisposable
    {
        ...
        private IDisposable? _modelIsExpandedSubscription;
        private bool _isExpanded;
        private bool _isUpdatingModelIsExpanded;
        ...
        public void Dispose()
        {
            _modelIsExpandedSubscription?.Dispose();
            _childRows?.Dispose();
        }
        ...
        private void Expand()
        {
            Expand(raiseNotifications: true, syncModel: true);
        }

        private void Expand(bool raiseNotifications, bool syncModel)
        {
            ...
            if (raiseNotifications && _isExpanded != oldExpanded)
        ...
        {
            Collapse(raiseNotifications: true, syncModel: true);
        }

        private void Collapse(bool raiseNotifications, bool syncModel)
        {
            var oldExpanded = _isExpanded;
        ...
        private void InitializeModelExpandedState()
        {
            if (_expanderColumn is not IModelExpanderState<TModel> modelExpanderState)
                return;

            if (modelExpanderState.TryReadModelIsExpanded(Model, out var isExpanded))
                ApplyModelExpandedState(isExpanded, raiseNotifications: false);

            if (modelExpanderState.CreateModelIsExpandedObservable(Model) is { } observable)
            {
                var ignoreInitial = true;
                _modelIsExpandedSubscription = observable.Subscribe(x =>
                {
                    if (!x.HasValue)
                        return;
                    ...
                    if (_isUpdatingModelIsExpanded || _isExpanded == x.Value)
                        return;

                    ApplyModelExpandedState(x.Value, raiseNotifications: true);
                });
            }
        }

        private void ApplyModelExpandedState(bool isExpanded, bool raiseNotifications)
        {
            if (_isExpanded == isExpanded)
                return;

            if (isExpanded)
                Expand(raiseNotifications, syncModel: false);
            else
                Collapse(raiseNotifications, syncModel: false);
        }

        private void SetModelIsExpanded()
        {
            if (_isUpdatingModelIsExpanded)
                return;

            try
            {
                _isUpdatingModelIsExpanded = true;
                _expanderColumn.SetModelIsExpanded(this);
            }
            finally
            {
                _isUpdatingModelIsExpanded = false;
            }
        }
