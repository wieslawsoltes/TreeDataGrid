...
{
    internal sealed class TreeDataGridRowSelectionInteraction<TModel> where TModel : class
    {
        ...
        private readonly ITreeDataGridRowSelectionModel<TModel> _selection;
        ...
        { _source = source; _selection = selection; }
        private bool SingleSelect => _selection.SingleSelect;
        private int Count => _selection.Count;
        private IndexPath SelectedIndex { get => _selection.SelectedIndex; set => _selection.SelectedIndex = value; }
        private IndexPath AnchorIndex => _selection.AnchorIndex;
        private IndexPath RangeAnchorIndex => _selection.RangeAnchorIndex;
        private bool IsSelected(IndexPath index) => _selection.IsSelected(index);
        private void Select(IndexPath index) => _selection.Select(index);
        private void Deselect(IndexPath index) => _selection.Deselect(index);
        private void Clear() => _selection.Clear();
        private IDisposable BatchUpdate()
        { _selection.BeginBatchUpdate(); return System.Reactive.Disposables.Disposable.Create(_selection.EndBatchUpdate); }
        public bool IsRowSelected(IRow rowModel)
        {
            ...
            if (!e.Handled)
            {
                ...
                if (direction.HasValue)
                {
                    ...
                    if (!e.Handled && direction == NavigationDirection.Left
                        && _source.IsHierarchical && anchorRowIndex > 0)
                    {
                        var newIndex = _source.Rows.ModelIndexToRowIndex(AnchorIndex[..^1]);
                    ...
                    if (!e.Handled && direction == NavigationDirection.Right
                       && _source.IsHierarchical && anchorRowIndex >= 0 && _source.Rows[anchorRowIndex] is IExpander { IsExpanded: true })
                    {
